...
def get_data_gen(data_dir, str2idx, batch_size=2):
    ...
    while True:
        ...
        for idx in range(0, len(files)):
            try:
                feature = np.loadtxt(data_dir+'/'+files[idx]+'.txt')
...
train_iter = get_data_gen(data_dir, get_str2idx(data_dir)[1], 40)
# data, label = next(train_iter)
# print(data, label)
model, model_data = createModel(len(get_str2idx(data_dir)[0])+2)

# check point
checkpointer = keras.callbacks.ModelCheckpoint(filepath="cnn-{epoch:02d}.hdf5", save_best_only=False, verbose=1,
                                               period=1)
model.load_weights('cnn-24.hdf5')
model.fit_generator(train_iter, steps_per_epoch=len(os.listdir(data_dir))//40, epochs=100,
